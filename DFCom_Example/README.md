...
> **当前主线运行方式：** 当前 `USER/main.c` 已接入 2026 电赛 H/D 固定路线，启动握手后等待 3 秒，自动运行所选路线一次。路线依赖 Pro 全量 Odom（0x80）回传，只有 VelPos 时会报 `ODOM_STALE` 并停止。先阅读 [路线与参数说明](../README.md#nuedc)。下文基础 API 教学仍可参考，原来的往返/转向主循环可在历史提交 `bd591db` 中查看。
